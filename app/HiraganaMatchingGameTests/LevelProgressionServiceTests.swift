...
struct LevelProgressionServiceTests {
    ...
    func levelCompletion() {
        ...
        #expect(service.isLevelUnlocked(2) == true)
        #expect(service.isLevelUnlocked(3) == true)
        #expect(service.isLevelUnlocked(4) == true)
    ...
    func levelProgressionStats() {
        ...
        #expect(stats.maxUnlockedLevel == 7) // 6スターで7レベルまで解放
    ...
    func nextLevelRecommendation() {
        ...
        // 途中レベルをスキップ
        service.completeLevel(5, earnedStars: 3)
        #expect(service.getRecommendedNextLevel() == 3) // 順序通りを推奨
    }
    ...
    func levelConfiguration() {
        ...
        #expect(level10Config.characters.count == 49) // 全文字（「ゐ」「ゑ」含む）
